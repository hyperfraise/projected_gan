...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    gw = np.clip(1280 // training_set.image_shape[2], 7, 32)
    gh = np.clip(736 // training_set.image_shape[1], 4, 32)
...
def training_loop(
    ...
    loss = dnnlib.util.construct_class_by_name(
        device=device, G=G, G_ema=G_ema, D=D, **loss_kwargs)  # subclass of training.loss.Loss
    phases = []
    ...
    fixed_gen_z = torch.randn(
        [2, G.z_dim], device=device)
    fixed_gen_c = [training_set.get_label(np.random.randint(
        len(training_set))) for _ in range(2)]
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for real_img, real_c, gen_z, gen_c in zip(phase_real_img, phase_real_c, phase_gen_z, phase_gen_c):
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_c=real_c,
                                          gen_z=gen_z, gen_c=gen_c, gain=phase.interval, cur_nimg=cur_nimg, real_images_batch=real_images_batch, fixed_gen_z=fixed_gen_z, fixed_gen_c=fixed_gen_c)
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            with torch.no_grad():
                ...
                vis_gen_c = np.stack(
                    [
                        fixed_gen_c[0] * (1 - t) + fixed_gen_c[1] * t
                        for t in np.arange(0, 1 + step, step)
                    ]
                )
                vis_images = []
                vis_gen_img = loss.run_G(vis_gen_z, vis_gen_c)
                vis_gen_img.data = vis_gen_img.data.mul(0.5).add(0.5)

                from moviepy.editor import ImageSequenceClip

                video_fake = vis_gen_img.mul(255).add_(0.5).clamp_(
                ...
                video_clips.write_videofile(
                    f"{run_dir}/transition_{cur_nimg//1000:06d}.mp4",
                    remove_temp=True,
                )
